...
mkdirSync('app/propiedades', { recursive: true })
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
          {['todas', 'venta', 'alquiler'].map(f => (
            <button key={f} onClick={() => setFiltro(f)} style={{ padding: '0.5rem 1.2rem', borderRadius: '8px', border: '1px solid #15803d', backgroundColor: filtro === f ? '#15803d' : 'white', color: filtro === f ? 'white' : '#15803d', cursor: 'pointer', fontWeight: 'bold', textTransform: 'capitalize' }}>
...
writeFileSync('lib/supabase.ts', supabase)
writeFileSync('app/propiedades/page.tsx', page)
console.log('Pagina de propiedades creada exitosamente')
